...
Tres notebooks, todos ejecutables en Google Colab, **completos, probados y validados**:
...
- **01 y 02:** solo **montan el Drive y leen** (no reconsolidan). Son independientes entre sí.
...
- Tiempos locales: 00 ≈ 5 min · 01 ≈ 2,5 min · 02 ≈ 20 s. En local, 01 y 02 leen de `datos_consolidados/`.

## 4. Dónde quedan los resultados

- **Local:** `datos_consolidados/` (regenerable) · `graficos_trayectorias/` + `.zip` (01) · `graficos_brechas/` + `.zip` (02).
...
- **Descargas de Colab:** `graficos_trayectorias.zip` (01) y `graficos_brechas.zip` (02).
...
- **Nombres de provincia cambian entre años** (" Buenos Aires", "Buenos aires", "Ciudad de Buenos Aires",
  "Tierra del Fuego, Antártida…") → `norm_prov()` en el 02.
